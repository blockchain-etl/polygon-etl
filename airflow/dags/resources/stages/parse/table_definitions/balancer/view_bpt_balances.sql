...
with bpt_transfers as (
    select 
      ...
      value,
      block_number
    FROM `blockchain-etl.polygon_balancer.V2_WeightedPool_event_Transfer` 
    union all
    select 
      contract_address as token_address,
      `from` as from_address,
      ...
      value,
      block_number
    FROM `blockchain-etl.polygon_balancer.V2_WeightedPool2Tokens_event_Transfer`
    union all
    select 
      contract_address as token_address,
      `from` as from_address,
      ...
      value,
      block_number
    FROM `blockchain-etl.polygon_balancer.V2_StablePool_event_Transfer`
    union all
    select 
      contract_address as token_address,
      `from` as from_address,
      ...
      value,
      block_number
    FROM `blockchain-etl.polygon_balancer.V2_MetaStablePool_event_Transfer`
),
...
calendar AS (
    select number as block_number from `public-data-finance.crypto_polygon.blocks`
),
running_balances as (
